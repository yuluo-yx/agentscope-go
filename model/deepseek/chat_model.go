...
// Package deepseek provides an OpenAI-compatible DeepSeek chat model.
package deepseek
...
type (
	// Credential configures DeepSeek authentication and endpoint settings.
	Credential = openai.Credential
	// CredentialOption customizes DeepSeek credentials.
	CredentialOption = openai.CredentialOption
	// ChatParameters configures DeepSeek chat completion parameters.
	ChatParameters = openai.ChatParameters
	// ChatModelOption customizes a DeepSeek ChatModel.
	ChatModelOption = openai.ChatModelOption
)

// ChatModel delegates DeepSeek's OpenAI-compatible API to the shared OpenAI adapter.
type ChatModel struct {
	inner *openai.ChatModel
}

// NewCredential creates DeepSeek credentials.
...
// WithBaseURL overrides the default DeepSeek OpenAI-compatible endpoint.
func WithBaseURL(baseURL string) CredentialOption { return openai.WithBaseURL(baseURL) }

// WithChatParameters sets default DeepSeek chat parameters.
...
// NewChatModel creates a DeepSeek chat model.
...
func (m *ChatModel) Call(ctx context.Context, request asmodel.CallRequest) (*asmodel.ChatResponse, error) {
	if m == nil || m.inner == nil {
		return nil, fmt.Errorf("deepseek: nil chat model")
	}
...
func (m *ChatModel) CountTokens(request asmodel.CallRequest) (int, error) {
	if m == nil || m.inner == nil {
		return 0, fmt.Errorf("deepseek: nil chat model")
	}
