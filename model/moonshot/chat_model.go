...
// Package moonshot provides an OpenAI-compatible Moonshot/Kimi chat model.
package moonshot
...
type (
	// Credential configures Moonshot authentication and endpoint settings.
	Credential = openai.Credential
	// CredentialOption customizes Moonshot credentials.
	CredentialOption = openai.CredentialOption
	// ChatParameters configures Moonshot chat completion parameters.
	ChatParameters = openai.ChatParameters
	// ChatModelOption customizes a Moonshot ChatModel.
	ChatModelOption = openai.ChatModelOption
)

// ChatModel delegates Moonshot's OpenAI-compatible API to the shared OpenAI adapter.
type ChatModel struct {
	inner *openai.ChatModel
}

// NewCredential creates Moonshot credentials.
...
// WithBaseURL overrides the default Moonshot OpenAI-compatible endpoint.
func WithBaseURL(baseURL string) CredentialOption { return openai.WithBaseURL(baseURL) }

// WithChatParameters sets default Moonshot chat parameters.
func WithChatParameters(parameters ChatParameters) ChatModelOption {
	return openai.WithChatParameters(parameters)
}

// WithStream configures whether Moonshot calls use streaming by default.
...
// NewChatModel creates a Moonshot chat model.
...
func (m *ChatModel) Call(ctx context.Context, request asmodel.CallRequest) (*asmodel.ChatResponse, error) {
	if m == nil || m.inner == nil {
		return nil, fmt.Errorf("moonshot: nil chat model")
	}
...
func (m *ChatModel) CountTokens(request asmodel.CallRequest) (int, error) {
	if m == nil || m.inner == nil {
		return 0, fmt.Errorf("moonshot: nil chat model")
	}
