...
// ModelCardType identifies the type of TTS model described by a card.
type ModelCardType string

const (
	// ModelCardTypeTTS describes a text-to-speech model.
	ModelCardTypeTTS ModelCardType = "tts_model"
)

// ModelStatus is the lifecycle status of a TTS model card.
...
// ModelCard describes a TTS model and its provider-facing metadata.
...
// CommonParameterSchema returns the shared TTS parameter schema.
...
// ParseModelCardYAML parses one Python-compatible YAML TTS card.
func ParseModelCardYAML(data []byte, baseSchema map[string]any) (ModelCard, error) {
	var raw ttsModelCardYAML
	...
	card := ModelCard{
		...
		Voices:             append([]string(nil), raw.Voices...),
		ParameterOverrides: cloneOverrideMap(raw.ParameterOverrides),
	}
	applyModelCardDefaults(&card)
	card.ParameterSchema = mergeParameterSchema(baseSchema, card.Voices, card.ParameterOverrides)
...
// LoadModelCardsFS loads all .yaml/.yml TTS cards from an fs.FS directory.
...
func (c ModelCard) Validate() error {
	if strings.TrimSpace(c.Name) == "" {
		return fmt.Errorf("tts model card: name is required")
	}
	if strings.TrimSpace(c.Label) == "" {
		return fmt.Errorf("tts model card %s: label is required", c.Name)
	}
	if c.Status != ModelStatusActive && c.Status != ModelStatusDeprecated && c.Status != ModelStatusSunset {
		return fmt.Errorf("tts model card %s: unsupported status %q", c.Name, c.Status)
	}
	return nil
}
...
func applyModelCardDefaults(card *ModelCard) {
	...
	if len(card.OutputTypes) == 0 {
		card.OutputTypes = []string{"audio/wav"}
	}
...
func mergeParameterSchema(baseSchema map[string]any, voices []string, overrides map[string]map[string]any) map[string]any {
	...
	if len(voices) > 0 {
		if voice, ok := properties["voice"].(map[string]any); ok {
			merged := utils.CloneAnyMap(voice)
			merged["default"] = voices[0]
			enum := make([]any, 0, len(voices))
			for _, voiceName := range voices {
				enum = append(enum, voiceName)
			}
			merged["enum"] = enum
			properties["voice"] = merged
		}
	}
