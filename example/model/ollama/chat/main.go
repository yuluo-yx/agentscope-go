...
import (
	...
	"github.com/yuluo-yx/agentscope-go/pkg/model/ollama"
...
func chat() {
	...
	fmt.Printf("ollama_live=ok response=%q\n", responseText)
	...
	if weatherCall == nil {
		...
		panic(fmt.Sprintf("ollama weather request returned no tool call: %q", text))
	}
	...
	fmt.Printf("ollama_weather=ok tool=%s input=%s response=%q\n", weatherCall.Name, weatherCall.Input, weatherText)
}

func streamChat() {

	streamChat := newOllamaModel(true)
	...
	for response := range responses {
		...
		if text != "" {
			...
			fmt.Printf("ollama_stream_delta=%q\n", text)
		}
	}
	if finalText == "" {
		finalText = streamed.String()
	}
	fmt.Printf("ollama_stream=ok response=%q\n", finalText)
}
