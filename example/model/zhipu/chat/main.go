...
func chat() {
	...
	fmt.Printf("glm_live=ok response=%q\n", responseText)
	...
	fmt.Printf("glm_weather=ok tool=%s input=%s response=%q\n", weatherCall.Name, weatherCall.Input, weatherText)
}

func streamChat() {

	streamChat := newGlmModel(true)
	...
	for response := range responses {
		...
		if text != "" {
			...
			fmt.Printf("glm_stream_delta=%q\n", text)
		}
	}
	if finalText == "" {
		finalText = streamed.String()
	}
	fmt.Printf("glm_stream=ok response=%q\n", finalText)
}

func newGlmModel(stream bool) asmodel.ChatModel {

	chat, err := zhipu.NewChatModel(
		zhipu.NewCredential(
			os.Getenv("AI_ZHIPU_API_KEY"),
		),
		"glm5.1",
