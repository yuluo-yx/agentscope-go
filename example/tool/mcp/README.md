...
The program starts a local in-process MCP server, connects it with `tool/mcp`, wraps the MCP tool as an AgentScope `tool.Tool`, registers it in a `tool.Toolkit`, runs it directly, then lets DashScope decide to call the MCP tool and sends the tool result back to the model.

## Prerequisites

- Go 1.26.3 or newer.
- `AI_DASHSCOPE_API_KEY` for the DashScope ChatModel tool-call loop.
...
Output includes the connected MCP client, the wrapped tool name, the direct MCP tool result, and the model-selected MCP tool call:
