...
func TestChatModelDelegatesCompatibleCallStreamAndTokenCounting(t *testing.T) {
	t.Parallel()
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if stream, _ := body["stream"].(bool); stream {
			...
			fmt.Fprint(writer, "data: {\"id\":\"dashscope-stream\",\"object\":\"chat.completion.chunk\",\"choices\":[{\"index\":0,\"delta\":{\"content\":\"dashscope stream\"},\"finish_reason\":\"stop\"}]}\n\n")
		...
		if err := json.NewEncoder(w).Encode(map[string]any{
			...
			"choices": []any{map[string]any{
				"index":         0,
				"message":       map[string]any{"role": "assistant", "content": "dashscope ok"},
				"finish_reason": "stop",
			}},
		}); err != nil {
	...
	if got := model.Name(); got != "dashscope:qwen-plus" {
	...
	if got := resp.GetTextContent(""); got == nil || *got != "dashscope ok" {
	...
	if body := <-requests; body["model"] != "qwen-plus" || body["stream"] != true {
...
func TestNilChatModelReportsDashScopeErrors(t *testing.T) {
	...
	var model *ChatModel
	if got := model.Name(); got != "dashscope:<nil>" {
		t.Fatalf("nil Name mismatch: %q", got)
	}
	if _, err := model.Call(context.Background(), asmodel.CallRequest{}); err == nil || !strings.Contains(err.Error(), "dashscope: nil chat model") {
		t.Fatalf("nil Call error mismatch: %v", err)
	}
	if _, err := model.Stream(context.Background(), asmodel.CallRequest{}); err == nil || !strings.Contains(err.Error(), "dashscope: nil chat model") {
		t.Fatalf("nil Stream error mismatch: %v", err)
	}
	if _, err := model.CountTokens(asmodel.CallRequest{}); err == nil || !strings.Contains(err.Error(), "dashscope: nil chat model") {
