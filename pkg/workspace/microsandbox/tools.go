...
func newBashTool(workspace *Workspace) tool.Tool {
	return &microsandboxTool{workspace: workspace, delegate: builtin.NewBash(), execute: executeBash(workspace)}
}

func newEditTool(workspace *Workspace) tool.Tool {
	return &microsandboxTool{workspace: workspace, delegate: builtin.NewEdit(), execute: executeEdit(workspace)}
}

func newGlobTool(workspace *Workspace) tool.Tool {
	return &microsandboxTool{workspace: workspace, delegate: builtin.NewGlob(), readOnly: true, concurrencySafe: true, execute: executeGlob(workspace)}
}

func newGrepTool(workspace *Workspace) tool.Tool {
	return &microsandboxTool{workspace: workspace, delegate: builtin.NewGrep(), readOnly: true, concurrencySafe: true, execute: executeGrep(workspace)}
}

func newReadTool(workspace *Workspace) tool.Tool {
	return &microsandboxTool{workspace: workspace, delegate: builtin.NewRead(), readOnly: true, concurrencySafe: true, execute: executeRead(workspace)}
}

func newWriteTool(workspace *Workspace) tool.Tool {
	return &microsandboxTool{workspace: workspace, delegate: builtin.NewWrite(), execute: executeWrite(workspace)}
}

func (t *microsandboxTool) Name() string {
	return t.delegate.Name()
}

func (t *microsandboxTool) Description() string {
	return t.delegate.Description() + " The operation runs inside the Microsandbox workspace microVM."
}

func (t *microsandboxTool) InputSchema() map[string]any {
	return t.delegate.InputSchema()
}

func (t *microsandboxTool) IsConcurrencySafe() bool {
	return t.concurrencySafe
}

func (t *microsandboxTool) IsReadOnly() bool {
	return t.readOnly
}

func (t *microsandboxTool) IsExternalTool() bool {
	return false
}

func (t *microsandboxTool) IsStateInjected() bool {
	return false
}

func (t *microsandboxTool) IsMCP() bool {
	return false
}

func (t *microsandboxTool) MCPName() string {
	return ""
}

func (t *microsandboxTool) CheckPermissions(ctx context.Context, input map[string]any, permissionCtx *permission.Context) (*permission.Decision, error) {
	return t.delegate.CheckPermissions(ctx, input, permissionCtx)
}

func (t *microsandboxTool) MatchRule(ruleContent string, input map[string]any) bool {
	return t.delegate.MatchRule(ruleContent, input)
}

func (t *microsandboxTool) GenerateSuggestions(input map[string]any) []permission.Rule {
	return t.delegate.GenerateSuggestions(input)
}

func (t *microsandboxTool) Execute(ctx context.Context, input map[string]any, state *asstate.AgentState) (<-chan tool.ToolChunk, error) {
	if t.workspace == nil || t.workspace.handle == nil || !t.workspace.alive {
		return errorText("Error: Microsandbox workspace is not initialized."), nil
	}
...
func executeWrite(workspace *Workspace) func(context.Context, map[string]any, *asstate.AgentState) (<-chan tool.ToolChunk, error) {
	return func(ctx context.Context, input map[string]any, _ *asstate.AgentState) (<-chan tool.ToolChunk, error) {
		...
		return successText(fmt.Sprintf("The file %s has been written successfully inside the Microsandbox workspace (%d lines).", filePath, lineCount)), nil
	}
}
...
func executeEdit(workspace *Workspace) func(context.Context, map[string]any, *asstate.AgentState) (<-chan tool.ToolChunk, error) {
	return func(ctx context.Context, input map[string]any, _ *asstate.AgentState) (<-chan tool.ToolChunk, error) {
		...
		return successText(fmt.Sprintf("Successfully replaced %s in %s inside the Microsandbox workspace.", replacementMsg, filePath)), nil
	}
}
