...
func newBashTool(workspace *Workspace) tool.Tool {
	return &daytonaTool{workspace: workspace, delegate: builtin.NewBash(), execute: executeBash(workspace)}
}

func newEditTool(workspace *Workspace) tool.Tool {
	return &daytonaTool{workspace: workspace, delegate: builtin.NewEdit(), execute: executeEdit(workspace)}
}

func newGlobTool(workspace *Workspace) tool.Tool {
	return &daytonaTool{workspace: workspace, delegate: builtin.NewGlob(), readOnly: true, concurrencySafe: true, execute: executeGlob(workspace)}
}

func newGrepTool(workspace *Workspace) tool.Tool {
	return &daytonaTool{workspace: workspace, delegate: builtin.NewGrep(), readOnly: true, concurrencySafe: true, execute: executeGrep(workspace)}
}

func newReadTool(workspace *Workspace) tool.Tool {
	return &daytonaTool{workspace: workspace, delegate: builtin.NewRead(), readOnly: true, concurrencySafe: true, execute: executeRead(workspace)}
}

func newWriteTool(workspace *Workspace) tool.Tool {
	return &daytonaTool{workspace: workspace, delegate: builtin.NewWrite(), execute: executeWrite(workspace)}
}

func (t *daytonaTool) Name() string {
	return t.delegate.Name()
}

func (t *daytonaTool) Description() string {
	return t.delegate.Description() + " The operation runs inside the Daytona workspace sandbox."
}

func (t *daytonaTool) InputSchema() map[string]any {
	return t.delegate.InputSchema()
}

func (t *daytonaTool) IsConcurrencySafe() bool {
	return t.concurrencySafe
}

func (t *daytonaTool) IsReadOnly() bool {
	return t.readOnly
}

func (t *daytonaTool) IsExternalTool() bool {
	return false
}

func (t *daytonaTool) IsStateInjected() bool {
	return false
}

func (t *daytonaTool) IsMCP() bool {
	return false
}

func (t *daytonaTool) MCPName() string {
	return ""
}

func (t *daytonaTool) CheckPermissions(ctx context.Context, input map[string]any, permissionCtx *permission.Context) (*permission.Decision, error) {
	return t.delegate.CheckPermissions(ctx, input, permissionCtx)
}

func (t *daytonaTool) MatchRule(ruleContent string, input map[string]any) bool {
	return t.delegate.MatchRule(ruleContent, input)
}

func (t *daytonaTool) GenerateSuggestions(input map[string]any) []permission.Rule {
	return t.delegate.GenerateSuggestions(input)
}

func (t *daytonaTool) Execute(ctx context.Context, input map[string]any, state *asstate.AgentState) (<-chan tool.ToolChunk, error) {
	if t.workspace == nil || t.workspace.handle == nil || !t.workspace.alive {
		return errorText("Error: Daytona workspace is not initialized."), nil
	}
...
func executeWrite(workspace *Workspace) func(context.Context, map[string]any, *asstate.AgentState) (<-chan tool.ToolChunk, error) {
	return func(ctx context.Context, input map[string]any, _ *asstate.AgentState) (<-chan tool.ToolChunk, error) {
		...
		return successText(fmt.Sprintf("The file %s has been written successfully inside the Daytona workspace (%d lines).", filePath, lineCount)), nil
	}
}
...
func executeEdit(workspace *Workspace) func(context.Context, map[string]any, *asstate.AgentState) (<-chan tool.ToolChunk, error) {
	return func(ctx context.Context, input map[string]any, _ *asstate.AgentState) (<-chan tool.ToolChunk, error) {
		...
		return successText(fmt.Sprintf("Successfully replaced %s in %s inside the Daytona workspace.", replacementMsg, filePath)), nil
	}
}
