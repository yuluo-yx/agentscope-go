...
func TestPathHelpersHandleEdgeCases(t *testing.T) {
	...
	relative, err := normalizeSandboxPath(" report.txt ", "/home/daytona/project")
	if err != nil || relative != "/home/daytona/project/report.txt" {
		t.Fatalf("normalizeSandboxPath relative = %q, %v", relative, err)
	}
	absolute, err := normalizeSandboxPath("/tmp/../data.txt", "/home/daytona/project")
	if err != nil || absolute != "/data.txt" {
		t.Fatalf("normalizeSandboxPath absolute = %q, %v", absolute, err)
	}
	if _, err := normalizeSandboxPath(" ", "/home/daytona/project"); err == nil || !strings.Contains(err.Error(), "empty") {
		t.Fatalf("normalizeSandboxPath empty error = %v", err)
	}
	if _, err := normalizeSandboxPath("bad\x00path", "/home/daytona/project"); err == nil || !strings.Contains(err.Error(), "NUL") {
		t.Fatalf("normalizeSandboxPath NUL error = %v", err)
	}
...
func TestWorkspaceToolsHandleAdditionalBranches(t *testing.T) {
	...
	handle.files["/home/daytona/notes.txt"] = []byte(longLine + "\nsecond\n")
	handle.files["/home/daytona/repeated.txt"] = []byte("foo\nfoo\n")
	...
	if replaceAll.State != message.ToolResultSuccess || string(handle.files["/home/daytona/repeated.txt"]) != "bar\nbar\n" {
		t.Fatalf("replace-all edit = %s %q file=%q", replaceAll.State, textOutput(replaceAll), handle.files["/home/daytona/repeated.txt"])
	}

	glob := findTool(t, ws, "Glob")
	handle.runResult = runResult{Stdout: "/home/daytona/a.txt\n", ExitCode: 0}
	...
	if got, err := globBaseDir(map[string]any{}, "/home/daytona"); err != nil || got != "/home/daytona" {
		t.Fatalf("globBaseDir default = %q, %v", got, err)
	}
	if got, err := grepSearchPath(map[string]any{}, "/home/daytona"); err != nil || got != "/home/daytona" {
	...
	if writePathError.State != message.ToolResultError || !strings.Contains(textOutput(writePathError), "empty") {
	...
	writeErrHandle.files["/home/daytona/notes.txt"] = []byte("old")
	...
	handle.runResult = runResult{Stdout: "/home/daytona/app.py:1:needle\n", ExitCode: 0}
